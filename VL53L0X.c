...
uint16_t readRangeContinuousMillimeters(VL53L0X * lidar)
{
    ...
    setTimeout(lidar, 2000); // Timeout set to 2000 milliseconds (2 seconds)
    ...
    // Reset the timeout to its default value after successful measurement
    setTimeout(lidar,2000); // Set the timeout back to default
...
uint16_t readRangeSingleMillimeters(VL53L0X * lidar)
{
  ...
  while (readReg(lidar,SYSRANGE_START) & 0x01)
  {
    ...
    {
      lidar->did_timeout = true;
      HAL_UART_Transmit(&huart3,"time out1",9, 9);
      return 0;
    }
  }
